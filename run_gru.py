...
from src import DEFAULT_CONFIG, load_coin, compute_features, CryptoDataset, fit_transform_splits, GRUForecaster, train, compute_metrics, print_results, plot_predictions

def run_gru_model(coin_config, config=DEFAULT_CONFIG):
    name = coin_config['name']
    print(f"\n{'='*10} Processing {name} (GRU) {'='*10}")
    
    filepath = f"data/{coin_config['file']}"
    df = load_coin(filepath)
    df = compute_features(df, feature_set='basic', window_size=config.window_size)
    ...
    X_train, y_train, X_val, y_val, X_test, y_test, scaler_y = fit_transform_splits(
        ...
        feature_cols=config.basic_features,
        target_col='log_ret',
