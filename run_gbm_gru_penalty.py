...
from src import DEFAULT_CONFIG, load_coin, compute_features, CryptoDataset, fit_transform_splits, GRUForecaster, DirectionalLoss, train, compute_metrics, print_results, plot_predictions

def run_gbm_gru_penalty_model(coin_config, config=DEFAULT_CONFIG):
    name = coin_config['name']
    print(f"\n{'='*10} Processing {name} (GBM Features, GRU, Directional Loss) {'='*10}")
    ...
    criterion = DirectionalLoss(penalty_weight=5.0)
